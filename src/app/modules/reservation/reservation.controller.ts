...
const barberReservation = catchAsync(async (req: Request, res: Response) => {
  const reservation = await ReservationService.barberReservationFromDB(
    req.user,
...
const customerReservation = catchAsync(async (req: Request, res: Response) => {
  const reservation = await ReservationService.customerReservationFromDB(
    req.user,
    req.query.status as string,
  );
  sendResponse(res, {
...
const reservationSummerForBarber = catchAsync(
  async (req: Request, res: Response) => {
    const reservation =
      await ReservationService.reservationSummerForBarberFromDB(req.user);
    sendResponse(res, {
      statusCode: StatusCodes.OK,
      success: true,
      message: 'Reservation created successfully',
...
const reservationDetails = catchAsync(async (req: Request, res: Response) => {
  const reservation = await ReservationService.reservationDetailsFromDB(
    req.params.id,
  );
  sendResponse(res, {
    statusCode: StatusCodes.OK,
...
const respondedReservation = catchAsync(async (req: Request, res: Response) => {
  const reservation = await ReservationService.respondedReservationFromDB(
    req.params.id,
    req.query.status as string,
  );
  sendResponse(res, {
