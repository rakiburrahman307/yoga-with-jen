...
const barberReservationFromDB = async (
  user: JwtPayload,
  status: string,
): Promise<{}> => {
  const condition: any = {
  ...
  const allStatus = await Promise.all(
    ['Upcoming', 'Accepted', 'Canceled', 'Completed'].map(
      async (status: string) => {
        return {
          status,
...
const customerReservationFromDB = async (
  user: JwtPayload,
  status: string,
): Promise<IReservation[]> => {
  const condition: any = {
...
const reservationSummerForBarberFromDB = async (
  user: JwtPayload,
): Promise<{}> => {
  // total earnings
...
const reservationDetailsFromDB = async (
  id: string,
): Promise<{ reservation: IReservation | null; report: any }> => {
...
const respondedReservationFromDB = async (
  id: string,
  status: string,
