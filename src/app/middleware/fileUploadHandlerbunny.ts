...
const fileUploadHandlerbunny = async (
  req: Request,
  res: Response,
  next: NextFunction,
): Promise<void> => {
  upload(req, res, async (error: any) => {
    ...
    if (req.files) {
      ...
      if (files['image']) {
        ...
        fileUrls.imageUrl = await BunnyStorageHandeler.uploadToBunny(
          imageFile,
          'images',
        );
      }
      ...
      if (files['thumbnail']) {
        ...
        fileUrls.thumbnailUrl = await BunnyStorageHandeler.uploadToBunny(
          thumbnailFile,
          'thumbnails',
        );
      }
      ...
      if (files['document']) {
        ...
        fileUrls.documentUrl = await BunnyStorageHandeler.uploadToBunny(
          documentFile,
          'documents',
        );
      }
      ...
      if (files['audio']) {
        ...
        fileUrls.audioUrl = await BunnyStorageHandeler.uploadToBunny(
          audioFile,
          'audio',
        );
      }
      if (req.body.data) {
        try {
          ...
          return next(
            new AppError(
              StatusCodes.BAD_REQUEST,
              'Invalid JSON format in req.body.data',
            ),
          );
        }
      } else {
