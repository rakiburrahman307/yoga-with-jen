    ...
    const convertVideoToMov = async (videoBuffer: Buffer, originalName: string): Promise<Buffer> => {
        ...
        return new Promise<Buffer>((resolve, reject) => {
            try {
                ...
                //     '-crf', '30',                   // CRF for quality vs speed tradeoff
                //     '-c:a', 'copy',                 // Copy audio stream (no re-encoding)
                //     '-movflags', '+faststart',      // Optimize for web streaming
                //     '-f', 'mov',                    // Output format (MOV)
                //     '-y',                           // Overwrite output file
                //     '-threads', '0',                // Use all available CPU threads
                //     '-vf', 'scale=1280:720',        // Optional: resize to 720p (reduce resolution for faster processing)
                //     '-progress', 'pipe:1',          // Output progress info
                //     '-loglevel', 'error',           // Show only errors in the logs
